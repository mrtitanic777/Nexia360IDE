...
if exist "dist\win-unpacked\NexiaIDE.exe" (
...
if errorlevel 1 (
    ...
    echo         Run directly from: dist\win-unpacked\NexiaIDE.exe
) else (
    echo    [OK] Deployed to Desktop\Nexia IDE\
    echo    [OK] Run: "%DEPLOY_DIR%\NexiaIDE.exe"
)
...
echo oLink.TargetPath = "%DEPLOY_DIR%\NexiaIDE.exe" >> "%TEMP%\nexia-shortcut.vbs"
