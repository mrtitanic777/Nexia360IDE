...
function registerIpcHandlers() {
    ...
    ipcMain.handle(IPC.XEX_INSPECT, async (_e, xexPath?: string) => {
        ...
        // nexia-core reads it. What was here was 241 lines of big-endian XEX
        // wrapping a little-endian PE, inside the one file that imports
        // Electron — a pure function of bytes that had no business living
        // beside the window and the menus. core/test/xex-parity.js proves the C
        // emits the same structure field for field against a real XEX.
        try {
            const core = path.join(__dirname, '..', 'nexia-core.exe');
            const out = execFileSync(core, ['xex', 'inspect', filePath],
                { encoding: 'utf8', windowsHide: true, maxBuffer: 32 * 1024 * 1024 });
            return JSON.parse(out);
        } catch (err: any) {
            // A malformed XEX is not an error here: nexia-core reports
            // valid:false with a reason, and exits 0. Reaching this means the
